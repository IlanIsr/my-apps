...
- `@repo/heroui` — the **override seam for HeroUI v3** (`@heroui/react`). Re-exports components per-file (`@repo/heroui/button` → HeroUI's `Button`, unchanged for now) so apps import from here, never `@heroui/react` directly, and the implementation can be wrapped/restyled/swapped in one place later. HeroUI v3 ships **prebuilt CSS** (no Tailwind plugin) — a consuming app adds `@import "@heroui/styles";` to its `globals.css` after `tailwindcss` (app-1 does; `@heroui/styles` is a direct devDep there) and no provider is needed. Used in app-1 by `AnniversaryForm`'s submit button; it's intentionally un-themed to the parchment palette for now. The full HeroUI stylesheet is ~420KB — switch to per-component CSS (`@heroui/styles/components/*.css`) if that matters.
