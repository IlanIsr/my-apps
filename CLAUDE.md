...
- **Migration**: `pnpm --filter @repo/anniversaries migrate` (dry run) / `--commit` (write). Needs `GOOGLE_*` + `FIREBASE_*` in the root `.env`. Groups existing tagged events into `persons` docs, rewrites future events' descriptions to `extendedProperties.private.personId`; leaves past events alone.
...
1. Finish the anniversaries → Firestore migration (branch `firestore-anniversaries`): user creates the named DB + service account + sets `FIREBASE_*`, runs `migrate --commit`, then merge + set the vars in App Hosting.
2. Build app-2's Firestore feature using the client-SDK pattern above.
3. After PR #50502 merges: configure `app1`/`app2.ilanisr.is-a.dev` subdomains via Firebase App Hosting.
